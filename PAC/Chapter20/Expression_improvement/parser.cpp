 ...
 * itself recursively to read hat subexpression as a unit.
 */
...
Expression *readT(TokenScanner & scanner) {
   ...
   if (type == WORD){
       if(contest.isFun(token)){
           string str=scanner.nextToken();
           if(str!="(") error("syntax error:expecting '(',unexpected "+str);
           Expression* tmp=readE(scanner,0);
           str=scanner.nextToken();
           if(str!=")") error("Unbalanced parentheses");
           return new funExp(tmp,token);
       }
